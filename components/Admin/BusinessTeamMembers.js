...
const BusinessTeamMembers = ({ businessId }) => {
  ...
  useEffect(() => {
    if (selectedUser) {
      console.log('Fetching availability for user ID:', selectedUser.id);
  ...
  const fetchUsers = async (businessId) => {
    ...
    try {
      ...
      if (businessDoc.exists()) {
        ...
        console.error('No business document found for the given ID:', businessId);
        setError('No business document found.');
        setLoading(false);
      }
    } catch (error) {
  ...
  const fetchAvailability = (userId) => {
    try {
      const businessesQuery = query(
        ...
        where(`uidTeamAvailability.${userId}`, '!=', null)
      );
  ...
  const handleSetAvailability = async (e) => {
    ...
    try {
      const businessesQuery = query(
        collection(db, 'businesses'), 
        where(`uidTeamAvailability.${selectedUser.id}`, '!=', null)
      );
